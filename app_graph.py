...
# 加入節點（含 wiki_link）
for _, row in persons.iterrows():
    net.add_node(f"person_{row['id']}", label=row['name'], title=row['name'], color="skyblue", shape="dot", href=row.get('wiki_link', '') or None)

for _, row in eras.iterrows():
    net.add_node(f"era_{row['id']}", label=row['era_name'], title=row['era_name'], color="lightgreen", shape="box", href=row.get('wiki_link', '') or None)

for _, row in locations.iterrows():
    net.add_node(f"location_{row['id']}", label=row['location_name'], title=row['location_name'], color="orange", shape="triangle", href=row.get('wiki_link', '') or None)

for _, row in events.iterrows():
    net.add_node(f"event_{row['id']}", label=row['event_name'], title=row['event_name'], color="violet", shape="ellipse", href=row.get('wiki_link', '') or None)

for _, row in objects.iterrows():
    net.add_node(f"object_{row['id']}", label=row['object_name'], title=row['object_name'], color="pink", shape="hexagon", href=row.get('wiki_link', '') or None)
...
for _, row in person_person.iterrows():
    net.add_edge(f"person_{row['person_id']}", f"person_{row['related_person_id']}")
