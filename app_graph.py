...
for _, row in persons.iterrows():
    ...
    net.add_node(row["person_id"], label=row["name"], title=title, color="skyblue", size=30)

# 加入事件節點（放大）
for _, row in events.iterrows():
    link = row["wiki_link"] if pd.notna(row["wiki_link"]) else "#"
    title = f"<a href='{link}' target='_blank'>事件：{row['event_name']}</a>"
    net.add_node(row["event_id"], label=row["event_name"], title=title, color="orange", size=20)

# 加入地點節點（放大）
for _, row in locations.iterrows():
    link = row["wiki_link"] if pd.notna(row["wiki_link"]) else "#"
    title = f"<a href='{link}' target='_blank'>地點：{row['location_name']}</a>"
    net.add_node(row["location_id"], label=row["location_name"], title=title, color="lightgreen", size=20)

# 加入物件節點（適中）
for _, row in objects.iterrows():
    link = row["wiki_link"] if pd.notna(row["wiki_link"]) else "#"
    title = f"<a href='{link}' target='_blank'>物件：{row['object_name']}</a>"
    net.add_node(row["object_id"], label=row["object_name"], title=title, color="violet", size=18)

# 加入時代表節點（中等大小）
for _, row in eras.iterrows():
    link = row["wiki_link"] if pd.notna(row["wiki_link"]) else "#"
    title = f"<a href='{link}' target='_blank'>時代：{row['era_name']}</a>"
    net.add_node(row["era_id"], label=row["era_name"], title=title, color="lightgray", size=22)
