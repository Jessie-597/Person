...
for _, row in persons.iterrows():
    net.add_node(row["person_id"], label=row["name"], title=row["alias"], color="#ff9999", shape="dot")

# 加入時代節點
eras = pd.read_sql_query("SELECT * FROM eras", conn)
for _, row in eras.iterrows():
    net.add_node(f"era_{row['era_id']}", label=row["era"], title=row["era"], color="#9999ff", shape="box")

# 加入事件節點
events = pd.read_sql_query("SELECT * FROM events", conn)
for _, row in events.iterrows():
    net.add_node(f"event_{row['era_id']}", label=row["event_name"], title=row["summary"], color="#66cc66", shape="diamond")

# 加入地點節點
locations = pd.read_sql_query("SELECT * FROM locations", conn)
for _, row in locations.iterrows():
    net.add_node(f"loc_{row['era_id']}", label=row["name"], title=row["type"], color="#ffcc66", shape="triangle")

# 加入物件節點
objects = pd.read_sql_query("SELECT * FROM objects", conn)
for _, row in objects.iterrows():
    net.add_node(f"obj_{row['era_id']}", label=row["name"], title=row["description"], color="#cc99ff", shape="ellipse")
